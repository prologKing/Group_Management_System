...
<%@page import="com.gpms.pojo.*, java.util.*,  java.text.SimpleDateFormat, org.apache.commons.lang3.StringUtils, java.time.LocalDate, java.time.LocalDateTime, java.time.format.DateTimeFormatter" contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE html>

<%
    List<ElectionType> userlist = null;
...
// Output "2018-05-12T17:21:53.658"
 ...
 try{
       ...
       userlist = loginCred.fetchElection();
       DashboardModel dash = new DashboardModel();
 
        
          if(request.getParameter("editcontribution") != null){
            
            ElectionType newEv = new ElectionType();
            DashboardModel dashboard = new DashboardModel();
          String contributionid = request.getParameter("contributionid");
         newEv.setId(Integer.parseInt(contributionid)); 
         String name = request.getParameter("name");
         newEv.setElectionName(name); 
          String electiondate = request.getParameter("contribution");
          DateTimeFormatter h = DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss");
          DateTimeFormatter g = DateTimeFormatter.ofPattern("yyyy-MM-dd");
          LocalDateTime e_date = LocalDateTime.parse(electiondate, h);
          newEv.setElectionStartDate(e_date.toString());
           String endDate = request.getParameter("end_date");
          LocalDateTime end_date = LocalDateTime.parse(endDate, h);
          newEv.setElectionEndDate(end_date.toString());
          String opening_date = request.getParameter("opening_date");
           LocalDate o_date = LocalDate.parse(opening_date, g);
          newEv.setFormOpeningDate(o_date.toString());
           String closing_date = request.getParameter("closing_date");
           LocalDate c_date = LocalDate.parse(closing_date, g);
          newEv.setFormClosingDate(c_date.toString());
        

          String res = dashboard.UpdateElectionRecord( newEv, (String)session.getAttribute("username"));
          ...
          if(request.getParameter("deletecontribution") != null){
            DashboardModel dashboard = new DashboardModel();
          String contributionid = request.getParameter("contributionid");
          String res = dashboard.deleteElectionRecord(Integer.parseInt(contributionid));
    ...
    <body>
            ...
            <div class="sidebar" data-color="purple" data-background-color="white" data-image="assets/img/sidebar-1.jpg">
                ...
                <div class="sidebar-wrapper">
                    <ul class="nav">
                        ...
                        <li style="display: <%=li10%>;" class="nav-item active">
                ...
                <nav class="navbar navbar-expand-lg navbar-transparent navbar-absolute fixed-top ">
                    <div class="container-fluid">
                        <div class="navbar-wrapper">
                            <a class="navbar-brand" href="#pablo">Elections</a>
                        </div>
                            ...
                            <div class="col-md-12">
                                <div class="card">
                                    <div class="card-header card-header-primary">
                                        ...
                                        <h4 class="card-title ">List of Elections 
                                    ...
                                    <div class="card-body">
                                        <div>
                                          ...
                                          while (i < userlist.size()) {
                                               ev = (ElectionType)userlist.get(i);
                                                    %>
                                                    <tr>
                                                        <td>
                                                             <a href="./Nav?area=election-details&id=<%=ev.getId()%>">
                                                            <%=ev.getElectionName()%>
                                                             </a>

                                                        </td>
                                                        <td>
                                                            <%=ev.getElectionStartDate()%>
                                                        </td>
                                                        <td>
                                                            <%=ev.getElectionEndDate()%>
                                                        </td>
                                                       
                                                        <td>
                                                            <button type="button" data-toggle="modal" data-target="#emailModal" data-values='{"id":"<%=ev.getId()%>", "name":"<%=ev.getElectionName()%>", "date":"<%=ev.getElectionStartDate()%>" , "edate":"<%=ev.getElectionEndDate()%>" , "odate":"<%=ev.getFormOpeningDate()%>",  "cdate":"<%=ev.getFormClosingDate()%>"}'><i  class="material-icons">edit</i> </button>
                                                            <button type="button" data-toggle="modal" data-target="#deleteModal" data-objvalues='{"id":"<%=ev.getId()%>", "name":"<%=ev.getElectionName()%>"}'><i  class="material-icons">delete</i> </button>
                                                        </td>
                                        ...
                                        xmhtt.onreadystatechange = function () {
                                            if (xmhtt.readyState === 4 && xmhtt.status === 200) {
                                                ...
                                                if (document.getElementById("options").value !== "") {
                                                    ...
                                                    var ull = "./Nav?mid=" + document.getElementById("member_id").value;
        ...
        <script>
            ...
            function validate(){
                ...
                var election_Startdate = new Date(document.getElementById("contribution").value);
                var election_Enddate = new Date(document.getElementById("end_date").value);
                var opening_date = new Date(document.getElementById("opening_date").value);
                var closing_date = new Date(document.getElementById("closing_date").value);
                
                 if(election_Enddate < election_Startdate || election_Startdate <=opening_date || election_Startdate <= closing_date){
                    if(election_Enddate < election_Startdate){
                        alert("Election Start Date cannot be after Election End date");
                    }
                    if(election_Startdate <=opening_date){
                        alert("Candidate form opening Date cannot be after Election start date");
                    }
                    if(election_Startdate <= closing_date){
                        alert("Candidate form closing Date cannot be after Election start date");
                    }
                    return false;
                }else{
                        ...
                        <div class="modal-body">
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name ="contribution" id="contribution" class="form-control datetimepicker" required>
                                </div>
                                <div class="form-group">
                                 ...
                                 <div class="form-group">
                                    ...
                                    <input type="date" name ="opening_date" id="opening_date" min="<%=fo%>" class="form-control" required>
                                </div>
                                <div class="form-group">
                                    <label class="bmd-label-floating">Candidate Form Closing Date.</label>
                                    <input type="date" name ="closing_date" id="closing_date" min="<%=fo%>" class="form-control" required>
        ...
        <script>
            ...
            $('#emailModal').on('show.bs.modal', function (event) {
                ...
                var contribution = button.data('values').date;
                var end_date = button.data('values').edate;
                var odate = button.data('values').odate;
                var cdate = button.data('values').cdate;
                
//                var Difference_In_Time = new Date().getTime() - new Date(cdate).getTime(); 
//                var value = Math.round(Difference_In_Time / (1000 * 3600 * 24));
//                if(value < 0){
//                   In_Days = Math.abs(value); 
//                }
                ...
                modal.find('#opening_date').val(odate);
                modal.find('#closing_date').val(cdate);
                ...
                <div class="modal-content">
                    <form action="" method="post">
                        ...
                        <div class="modal-body">
                            ...
                            <div class="col-md-12">
                                <p>Are you sure you want to delete the <span name="membername" id="membername"></span> ?</p>
                                <input type="hidden" name ="contributionid" id="contributionid" class="form-control" required />
                            </div>
                            <div class="modal-footer">
                                <button type="button" class="btn btn-default" data-dismiss="modal"><i  class="material-icons">close</i></button>
